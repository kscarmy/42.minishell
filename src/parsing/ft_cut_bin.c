...
/*   Updated: 2022/06/26 16:47:00 by guderram         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_cut_bin_inter_malloc(t_data *data, t_token *t ,char *str, int ret) // str = &data->input[data->i], stra = premiere string
{
	...
	//("ft_cut_bin_inter_malloc : entree\n");
	...
	//("ft_cut_bin_inter_malloc : sortie\n");
	data->i = data->i + i;
	data->dol = 0;
}
...
int	ft_cut_bin_inter_size(t_data *data, char *str, char *stra) // str = &data->input[data->i], stra = premiere string
{
	...
	i = ft_space(str, 0);
	tmp = NULL;
	// //("ft_cut_bin_inter_size : entree\n");
	while (str && str[i] && ft_is_separator(str, i) == 0 && i < ft_str_size(str))
	{
		// //("ft_cut_bin_inter_size : while 1 : i %d str <%s>\n", i, &str[i]);
		i = i + ft_space(str, i);
		// //("ft_cut_bin_inter_size : while 2 : i %d str <%s>\n", i, &str[i]);
		tmp = ft_one_arg(data, i);
		ft_parse_input_interpret(data, i);
		// //("ft_cut_bin_inter_size : while : i %d str <%s>\n", i, &str[i]);
		if (data->dol == 1)
		{
			// //("ft_cut_bin_inter_size : while : ret 1 %d\n", ret);
			ret = ret + ft_cut_bin_inter_sub(tmp);
			// //("ft_cut_bin_inter_size : while : ret 1 %d\n", ret);
		}
		else
		{
			// //("ft_cut_bin_inter_size : while : ret 2 %d\n", ret);
			ret++;
			// //("ft_cut_bin_inter_size : while : ret 2 %d\n", ret);
		}
		i = i + ft_incre_one_arg(data, i);
		// //("ft_cut_bin_inter_size : while 3 : i %d str <%s>\n", i, &str[i]);
		i = i + ft_space(str, i);
		// //("ft_cut_bin_inter_size : while 4 : i %d str <%s>\n", i, &str[i]);
		ft_strdel(&tmp);
		data->dol = 0;
	}
	data->dol = 0;
...
int	ft_cut_bin_inter_sub(char *str) // retourne le nombre d'arguments dans une string
{
	...
	// i = ft_space(str, i);
	ret = 0;
	// //("ft_cut_bin_inter_sub : entree : str <%s>\n", str);
	// while (str && str[i] == ' ')
	// 	i++;
	if (str && str[i] != ' ')
		ret++;
	...
	while (str && str[i])
	{
		// //("ft_cut_bin_inter_sub : while 1 : str[i] <%s>\n", &str[i]);
		i = i + ft_space(str, i);
		// //("ft_cut_bin_inter_sub : while 2 : str[i] <%s>\n", &str[i]);
		if (str[i] != ' ')
			ret++;
		while (str && str[i] != ' ')
			i++;
		// //("ft_cut_bin_inter_sub : while 3 : str[i] <%s>\n", &str[i]);
		i = i + ft_space(str, i);
		// //("ft_cut_bin_inter_sub : while 4 : str[i] <%s>\n", &str[i]);
	}
	return (ret);
}
